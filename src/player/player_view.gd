...
# Pure presentation. Reads state from its parent Player (replicated) and
# renders: top-down class sprite (rotated to aim_dir), HP/MP bars, downed
# halo, charge bar, and per-class skill FX driven by the host's FX RPCs.
...
func _fx_age(kind: String) -> float:
	if not _player._fx_local.has(kind):
		return -1.0
	return _now() - float(_player._fx_local[kind].get("t", _now()))

func _fx(kind: String, key: String, default_value: Variant = null) -> Variant:
	if not _player._fx_local.has(kind):
		return default_value
	return _player._fx_local[kind].get(key, default_value)
...
func _draw_berserker_fx() -> void:
	...
	if td >= 0.0 and td < 0.4:
		...
		var start_pos: Vector2 = _fx("dash", "start", _player.global_position)
...
func _draw_mage_fx() -> void:
	...
	if tf >= 0.0 and tf < 0.45:
		...
		var pos: Vector2 = _fx("fireball", "pos", _player.global_position)
	...
	if tc >= 0.0 and tc < 0.35:
		...
		for i in pts.size():
			var p: Vector2 = pts[i] - _player.global_position
	...
	if tb >= 0.0 and tb < 0.4:
		...
		var from_pos: Vector2 = _fx("blink", "from", _player.global_position)
		var to_pos: Vector2 = _fx("blink", "to", _player.global_position)
...
func _draw_bard_fx() -> void:
	...
	if td >= 0.0 and td < 0.35:
		...
		var s: Vector2 = _fx("dash", "from", _player.global_position) - _player.global_position
...
func _draw_crossbow_fx() -> void:
	...
	if tr >= 0.0 and tr < 0.4:
		...
		var from: Vector2 = _fx("roll", "from", _player.global_position) - _player.global_position
