...
@onready var local_panel: VBoxContainer = $Root/BottomLeft/LocalPanel
...
@onready var skills_row: HBoxContainer = $Root/BottomLeft/LocalPanel/Skills
...
var _pending_upgrade_options: Array = []
...
func _process(_delta: float) -> void:
	_find_local_player()
	# Timer.
	var t: float = max(GameState.RUN_DURATION - GameState.run_time, 0.0)
	var mins := int(t) / 60
	var secs := int(t) % 60
	timer_label.text = "Time left: %d:%02d" % [mins, secs]
	# Party level / xp.
	...
	if _local_player != null and is_instance_valid(_local_player):
		local_nick.text = "%s (%s)" % [_local_player.nick, _local_player.klass]
		hp_bar.max_value = max(_local_player.max_hp, 1.0)
		hp_bar.value = _local_player.hp
		mp_bar.visible = _local_player.max_mp > 0
		if mp_bar.visible:
...
func _render_roster() -> void:
	# Lazy update once per second-ish.
	...
	for p in get_tree().get_nodes_in_group("players"):
		...
		lbl.text = "%s [%s] HP %d/%d%s" % [p.nick, p.klass, int(p.hp), int(p.max_hp), status]
		roster_box.add_child(lbl)

func show_upgrade_picks(options: Array) -> void:
	_pending_upgrade_options = options
...
func _local_pick_upgrade(id: String) -> void:
	...
	if arena != null and arena.has_method("submit_upgrade_pick"):
		arena.submit_upgrade_pick(id)

func show_endgame(won: bool) -> void:
