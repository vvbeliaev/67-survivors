...
#
# Usage:
#   bash scripts/deploy.sh <ip> <ssh-password> [port] [min-players]
#
# Example:
#   bash scripts/deploy.sh 1.2.3.4 mypassword 7777 2
#
...
IP="${1:?Usage: deploy.sh <ip> <ssh-password> [port] [min-players]}"
PASS="${2:?Usage: deploy.sh <ip> <ssh-password> [port] [min-players]}"
GAME_PORT="${3:-7777}"
MIN_PLAYERS="${4:-2}"
...
if ! command -v sshpass &>/dev/null; then
    echo "ERROR: sshpass not found locally."
...
SSH="sshpass -p $PASS ssh $SSH_OPT root@$IP"
...
echo "    Logs:  make logs IP=$IP PASS=***"
echo "    Stop:  make stop IP=$IP PASS=***"
